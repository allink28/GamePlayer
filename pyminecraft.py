...
def process_img(original_image):
    ...
    processed_img = cv2.Canny(processed_img, threshold1=150, threshold2=250) # 90,250
    processed_img = roi(processed_img)
    processed_img = cv2.GaussianBlur(processed_img, (5, 5), 0)
    # more info: http://docs.opencv.org/3.0-beta/doc/py_tutorials/py_imgproc/py_houghlines/py_houghlines.html
    #                         edges       rho   theta   thresh    lines     # min length, max gap:
    lines = cv2.HoughLinesP(processed_img, 1, np.pi / 180, 100, np.array([]),         20, 15)
